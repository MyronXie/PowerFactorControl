...
void LCD_DisNum2(uint16_t x, uint16_t y, uint32_t num, uint16_t color)
{
	LCD_DispChar((x+STR_WIDTH*0), y, (num/10)+'0', color);
	LCD_DispChar((x+STR_WIDTH*1), y, (num%10)+'0', color);
}
void LCD_DispADC(uint16_t x, uint16_t y, float num, uint16_t color)
{
		int i=0;
		int dig=0;
		while(num>=0)
		...
		num=num+1;
		dig--;
		LCD_DispChar(x+STR_WIDTH*0, y, dig+'0', color);
		LCD_DispChar(x+STR_WIDTH*1, y, '.', color);
		dig=0;
		for (i=0;i<3;i++)
		{
				num=num*10;
				...
				num=num+1;
				dig--;
				LCD_DispChar(x+STR_WIDTH*(2+i), y, dig+'0', color);
				dig=0;
		}
}
